...
Template.summernote.onRendered(function() {
  ...
  data = {};
  ['collection', 'imageCollection', 's3bucket', 's3subFolder', 'field', '_id', 'enabled'].forEach(function(key) {
  ...
  this.autorun(function() {
    Collection = Mongo.Collection.get(data.collection);
    $(".summernote").code(Collection.findOne(data._id)[data.field]);
  });
  ...
  this.autorun(function() {
    ...
    var newOptions = this.templateInstance().data.options || defaultOptions(this.templateInstance());
    if (!newOptions.onBlur){
      newOptions.onBlur = save;
    }
    if (newData.enabled) { //how can I check if summernote is already enabled?
      element.summernote(newOptions);
    } else {
      element.destroy();
    }
