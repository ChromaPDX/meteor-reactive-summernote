    ...
    this.autorun(function() {
      ...
      if (newImage) {
        ...
        if (bucket) {
          ...
          url = s3url(newImage, bucket, storeName);
          console.log('looking for amazon link: ', url);
        } else {
          url = newImage.url();
          console.log('looking for cfs link: ', url);
        }
        if (url) {
          self.uploadedImage.set(null);
          var templateInstance = self;
          var element = self.find('div.summernote');
          if (!element) return;
          element = $(element);
          element.summernote('insertImage', newImage.url(), newImage.original.name);
        }
      }
    });
  }
